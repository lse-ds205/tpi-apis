...
Current tests:
- Country data endpoint (/v1/country-structure/{country_code}/{year})
...
def test_get_valid_country_data(client, df_assessments):
    """Test GET endpoint /v1/country-structure/{country}/{year}.
    ...
    for country, year in zip(valid_requests["Country"], valid_requests["Year"]):
        response = client.get(f"/v1/country-structure/{country}/{year}")
        msg = (
...
def test_get_country_data_not_assessed(client, df_assessments):
    """Test GET endpoint /v1/country-structure/{country}/{year} for a country that has not been assessed.
    ...
    for country in not_assessed:
        response = client.get(f"/v1/country-structure/{country}/2024")
        msg = (
...
def test_get_a_json(client, df_assessments):
    """Test GET endpoint /v1/country-structure/{country}/{year} for valid country-year combinations.
            ...
            for area in pillar["areas"]:
                ...
                if area['name'] != 'CF.4':
                    ...
                    for indicator in area["indicators"]:
                        ...
                        expected_indicator_keys = ["name", "assessment"]
                        ...
                        if indicator["metrics"]:
                            metric = indicator["metrics"]

                            assert isinstance(metric, dict), base_error_msg + "The metric key should be a dict (even if an empty one)"
                            assert len(metric) == 2, base_error_msg + "The metric dictionary should have exactly 2 keys"
                            assert metric['name'] is not None, base_error_msg + "The metric dictionary should have a 'name' key"
                            assert metric['value'] is not None, base_error_msg + "The metric dictionary should have a 'value' key"